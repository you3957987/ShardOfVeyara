...
struct FBlackKnightWeightConfig
{
	GENERATED_BODY()
...
public:
	...
	TArray<FBlackKnightWeightConfig> CloseRangeWeights;

	// 중거리일 때 적용할 키-값 목록
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "자체설정", meta=(TitleProperty="Key"))
	TArray<FBlackKnightWeightConfig> MidRangeWeights;

	// 원거리일 때 적용할 키-값 목록
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "자체설정", meta=(TitleProperty="Key"))
	TArray<FBlackKnightWeightConfig> FarRangeWeights;
};
